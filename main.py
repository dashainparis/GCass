...
# Global list for translocations. Accumulated in memory while processing contigs and processed separately in the end.
all_translocations = []
...
chr_gem_dict = dict()
# value - contig string from fa file
chr_line_dict = dict()
contigs =[]
for data_file in data_files:
	len_cont = data_file.split('_')
	#logger.info(data_file)
	#logger.info(len_cont)
	if int(len_cont[5])<6000:
		continue
	else:
		...
		logger.info(str(len(ff))+' '+str(len(rr))+' '+ str(len(fr))+' '+str(len(rf)))
		contig = fr[0].first_read_chr
		logger.info('fr[0].first_read_chr',contig)
		contigs.append(contig)
		logger.info(contig)
		# Calculate different contig statistics
		...
		# file_length = open(config['working_dir'] + config['results_dir'] + contig + '_fragment_lengths' + '.txt', 'w')
		...
		# fr_wr = open(config['working_dir'] + config['results_dir'] + contig + '_abnormal_frag.txt','w')
		normal_fragments = []
		...
		fr_norm = open(config['working_dir'] + config['normal_fragments_dir'] + 'normal_fragments_' + contig + '.txt', 'w')
		...
		# write_begin_end_mid_files (fr_abn, rf_abn, rr_abn, ff_abn, contig, config)
		M = biggest_normal 
		chr_normal_fragments_dict[contig] = [i[0] for i in normal_fragments]	
		# Add stats for this contig for serialization
		stats_to_serialize['per_chr_stats'][contig] = dict()
		stats_to_serialize['per_chr_stats'][contig]['num_all_abn'] = abn
		stats_to_serialize['per_chr_stats'][contig]['R'] = R
		stats_to_serialize['per_chr_stats'][contig]['smallest_normal'] = smallest_normal
		stats_to_serialize['per_chr_stats'][contig]['biggest_normal'] = biggest_normal
		stats_to_serialize['per_chr_stats'][contig]['median'] = median
		#if not histogram_flag:
		#	stats_to_serialize['a'] = float(a)
		#	stats_to_serialize['b'] = float(b)
		#	stats_to_serialize['p'] = float(p)
		histogram_flag+=1
lambda_gc = lambdacalc.LambdaCalculation(config,chr_normal_fragments_dict,median)
# Serialize stats
